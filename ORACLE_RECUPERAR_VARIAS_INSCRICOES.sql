/*SESU - óbice sistêmico remanscente -recuperar vaga - FIES - Selecao Aluno
Ordem de trabalho # WO1336389*/
...
DECLARE
    ...
    P_CO_INSCRICAO               NUMBER(10);
    P_TP_USUARIO                 NUMBER(1)   := 2; --REGULAR OU REMANESCENTE
    P_TP_VAGA                    NUMBER(1)   := 3; --PUBLICO
    ...
    V_CO_AGENCIA          NUMBER;
    V_TP_FIANCA           NUMBER;
    V_QT_SEMESTRE_CURSO   NUMBER;
    V_QT_SEMESTRE_CONCLUIDO NUMBER;
    V_QT_SEMESTRE_FINANCIADO NUMBER;
    V_PC_SOLICITADO  NUMBER;
    V_VL_SEMESTRE_FIES NUMBER;
    V_VL_ATUAL_COM_DESCONTO NUMBER;
    V_VL_FINANCIADO_SEMESTRE NUMBER;
    V_VL_FINANCIADO_GLOBAL NUMBER;
    V_VL_LIMITE_GLOBAL NUMBER;
    V_VL_TAXA_JUROS NUMBER;
    V_NU_ABA NUMBER;
    V_PC_FINANCIAMENTO_MAXIMO NUMBER;
    V_CO_SEMESTRE_POSTERGAR NUMBER;
    V_CO_SEGURADORA NUMBER;
...
BEGIN
  ...
  FOR REC IN (
      ...
      SELECT      DISTINCT
        ...
        INNER JOIN  FIES_PREINSCRICAO.TB_PRE_USUARIO                 US  ON US.CO_USUARIO   = I.CO_USUARIO
        INNER JOIN  FIES_GLOBAL.TB_GLB_PESSOA_FISICA                 PF  ON PF.CO_PESSOA    = US.CO_PESSOA
        INNER JOIN  FIES_PREINSCRICAO.TB_PRE_CHAMADA_OCUPACAO        CO  ON CO.CO_INSCRICAO = I.CO_INSCRICAO AND 
                                                                            CO.CO_USUARIO   = I.CO_USUARIO   AND 
                                                                            CO.CO_SEMESTRE  = I.CO_SEMESTRE                                                             
        INNER JOIN  FIES_FINANCIAMENTO.TB_FIN_CANDIDATO              CD  ON CD.CO_USUARIO   = I.CO_USUARIO  
        ...
        AND         CO.DT_OCUPACAO           IS NULL
        AND         PF.NU_CPF IN ('10898571600', '10804448663', '13490654633')
        
        ) LOOP 
        
        P_CO_INSCRICAO := REC.CO_INSCRICAO;
        P_CO_SEMESTRE  := REC.CO_SEMESTRE; 
        P_CO_USUARIO   := REC.CO_USUARIO;
        P_TP_USUARIO   := REC.TP_USUARIO;
        P_NU_CPF       := REC.NU_CPF;
    ...
    IF V_RETORNO IS NULL THEN
        ...
        BEGIN
            ...
            SELECT  PI.CO_CLUSTER,  CCD.CO_H_CURSO,  TF.CO_CURSO,  TF.CO_TURNO
             ...
             WHERE 1 = 1
                AND TF.NU_OPCAO       = 1
                AND PI.CO_INSCRICAO   = REC.CO_INSCRICAO
                AND PI.CO_SEMESTRE    = REC.CO_SEMESTRE
                AND PI.CO_USUARIO     = REC.CO_USUARIO
                AND PI.TP_USUARIO     = REC.TP_USUARIO;

        EXCEPTION
            WHEN OTHERS THEN
            BEGIN
                V_RETORNO := '{"CPF":"'||REC.NU_CPF||'", "INSCRICAO": "'||REC.CO_INSCRICAO||'", "ERRO":"FALHA AO RECUPERAR A PRIMEIRA OPCAO DA TERMO"}';
        ...
        BEGIN
            ...
            IF V_EXISTE = 0 THEN
                --V_RETORNO := '{"CPF":"'||REC.NU_CPF||'", "INSCRICAO": "'||REC.CO_INSCRICAO||'", "ERRO":"FALHA AO RECUPERAR O TERMO DE FINANCIAMENTO - CURSO('||V_CO_CURSO||') TURNO('||V_CO_TURNO||')"}';
                SELECT 0 INTO V_VAGA_RECUPERADA FROM DUAL;
            END IF;
            --
        END;
    ...
    IF V_RETORNO IS NULL THEN
        ...
        BEGIN
            --V_RETORNO := '{"CPF":"'||REC.NU_CPF||'", "INSCRICAO": "'||REC.CO_INSCRICAO||'", "ERRO":"ERRO AO RECUPERAR UMA CADEIRA DISPONIVEL NO CURSO SELECIONADO PELO CANDIDATO NA INSCRICAO"}';
            ...
            EXCEPTION
                WHEN OTHERS THEN
                    V_RETORNO := '{"CPF":"'||REC.NU_CPF||'", "INSCRICAO": "'||REC.CO_INSCRICAO||'", "ERRO":"Falha ao gerar vaga de liminar ['||V_NU_LINHA||'] - cluster['||V_CO_CLUSTER||'] hcurso['||V_CO_H_CURSO||'] curso['||V_CO_CURSO||'] - turno['||V_CO_TURNO||'] - ['||SQLERRM||'] "}';
        END;
        END IF;
        ...
        BEGIN
            ...
            BEGIN
                ...
                V_GERA_LIMINAR := '{"CO_INSCRICAO":"'||REC.CO_INSCRICAO||'", "CO_LIMINAR":"'||V_CO_LIMINAR||'", "ERRO":"" }';
            EXCEPTION
                WHEN OTHERS THEN
                    V_GERA_LIMINAR := NULL;
            END;
            ...
            BEGIN
                ...
                VALUES     ( V_CO_LIMINAR, V_CO_PERFIL_USUARIO, SYSDATE, SYSDATE, V_ULTIMO_DIA, 'N',REC.CO_SEMESTRE );
                ...
                VALUES     ( FIES_PREINSCRICAO.SQ_PRE_HISTORICO_LIMINAR.NEXTVAL, V_CO_LIMINAR, 'N', V_DS_JUSTIFICATIVA, SYSDATE, V_ULTIMO_DIA, V_CO_PERFIL_USUARIO, SYSDATE,REC.CO_SEMESTRE );
                ...
                WHEN OTHERS THEN BEGIN
                    V_GERA_LIMINAR := '{"CO_INSCRICAO":"'||REC.CO_INSCRICAO||'", "CO_LIMINAR":"0", "ERRO":" FALHA AO CRIAR LIMINAR ('||SQLERRM||')" }';
                END;

            END;
            END IF;

            /* FUNCAO  SELECT FIES_PREINSCRICAO.FC_LIMINAR_RECUPERACAO_VAGA(REC.CO_INSCRICAO) INTO V_GERA_LIMINAR FROM DUAL;   */
            ...
            IF V_CO_LIMINAR = 0 THEN
                V_RETORNO := '{"CPF":"'||REC.NU_CPF||'", "INSCRICAO": "'||REC.CO_INSCRICAO||'", "ERRO":"Falha ao gerar liminar - ['||V_MSG_ERROR||'] "}';
            END IF;
    ...
    IF V_RETORNO IS NULL THEN
        ...
        IF V_TP_VAGA_LIMINAR = 1 THEN
            BEGIN
                ...
                EXCEPTION
                WHEN OTHERS THEN
                    V_RETORNO := '{"CPF":"'||REC.NU_CPF||'", "INSCRICAO": "'||REC.CO_INSCRICAO||'", "ERRO":"FALHA AO RECUPERAR VAGA PUBLICA ['||V_CO_VAGA_PUBLICO||'] - ['||SQLERRM||'] "}';
                 END;
            ...
            BEGIN
                ...
                EXCEPTION
                WHEN OTHERS THEN
                V_RETORNO := '{"CPF":"'||REC.NU_CPF||'", "INSCRICAO": "'||REC.CO_INSCRICAO||'", "ERRO":"FALHA AO RECRIAR VAGA PUBLICA ['||V_CO_VAGA_PUBLICO||'] - ['||SQLERRM||'] "}';
                END;
            END IF;
        END IF;
    ...
    BEGIN
            ...
            SELECT 
                        ...
                        V_TP_FIANCA,
                        V_QT_SEMESTRE_CURSO,
                        V_QT_SEMESTRE_CONCLUIDO,
                        V_QT_SEMESTRE_FINANCIADO,
                        V_PC_SOLICITADO,
                        V_VL_SEMESTRE_FIES,
                        V_VL_ATUAL_COM_DESCONTO,
                        V_VL_FINANCIADO_SEMESTRE,
                        V_VL_FINANCIADO_GLOBAL,
                        V_VL_LIMITE_GLOBAL,
                        V_VL_TAXA_JUROS,
                        V_NU_ABA,
                        V_PC_FINANCIAMENTO_MAXIMO,
                        V_CO_SEMESTRE_POSTERGAR,
                        V_CO_SEGURADORA
                        
        FROM (
                    SELECT * FROM FIES_AUDITORIA.LG_FIN_CANDIDATO_PUBLICO  
                    WHERE CO_USUARIO = REC.CO_USUARIO AND NU_ABA = 4 ORDER BY DT_OPERACAO_LOG desc
             )
        WHERE ROWNUM = 1;
  
        UPDATE FIES_FINANCIAMENTO.TB_FIN_CANDIDATO_PUBLICO SET
                        CO_AGENCIA = V_CO_AGENCIA,
                        TP_FIANCA = V_TP_FIANCA,
                        QT_SEMESTRE_CURSO = V_QT_SEMESTRE_CURSO,
                        QT_SEMESTRE_CONCLUIDO = V_QT_SEMESTRE_CONCLUIDO,
                        QT_SEMESTRE_FINANCIADO = V_QT_SEMESTRE_FINANCIADO,
                        PC_SOLICITADO = V_PC_SOLICITADO,
                        VL_SEMESTRE_FIES = V_VL_SEMESTRE_FIES,
                        VL_ATUAL_COM_DESCONTO = V_VL_ATUAL_COM_DESCONTO,
                        VL_FINANCIADO_SEMESTRE = V_VL_FINANCIADO_SEMESTRE,
                        VL_FINANCIADO_GLOBAL = V_VL_FINANCIADO_GLOBAL,
                        VL_LIMITE_GLOBAL = V_VL_LIMITE_GLOBAL,
                        VL_TAXA_JUROS = V_VL_TAXA_JUROS,
                        NU_ABA = V_NU_ABA,
                        PC_FINANCIAMENTO_MAXIMO = V_PC_FINANCIAMENTO_MAXIMO,
                        CO_SEMESTRE_POSTERGAR = V_CO_SEMESTRE_POSTERGAR,
                        CO_SEGURADORA = V_CO_SEGURADORA,
                        CO_PERFIL_USUARIO = V_CO_PERFIL_USUARIO
        WHERE  CO_USUARIO  = REC.CO_USUARIO;
        ...
        WHERE   CO_INSCRICAO            = REC.CO_INSCRICAO
          AND   CO_USUARIO              = REC.CO_USUARIO
          AND   CO_SEMESTRE             =REC.CO_SEMESTRE
          AND   TP_USUARIO              = REC.TP_USUARIO
        ...
        UPDATE FIES_FINANCIAMENTO.TB_FIN_CANDIDATO
           SET  DT_INICIO_FINANCIAMENTO   = SYSDATE,
                DT_FIM_FINANCIAMENTO      = ( SELECT TO_DATE(TO_CHAR(TRUNC(FIES_GLOBAL.SOMA_DIAS_UTEIS(SYSDATE, 7)), 'DD/MM/YYYY') || ' 23:59:59', 'DD/MM/YYYY HH24:MI:SS') FROM DUAL ),
                DT_LIMITE_CPSA            = ( SELECT TO_DATE(TO_CHAR(TRUNC(FIES_GLOBAL.SOMA_DIAS_UTEIS(SYSDATE, 10)), 'DD/MM/YYYY') || ' 23:59:59', 'DD/MM/YYYY HH24:MI:SS') FROM DUAL ),
                CO_SEMESTRE_FINANCIAMENTO = P_CO_SEMESTRE_FINANCIAMENTO,
                CO_SEMESTRE               =REC.CO_SEMESTRE,
                TP_USUARIO                = REC.TP_USUARIO,
                TP_VAGA                   = V_TP_VAGA_LIMINAR,
                CO_INSCRICAO              = REC.CO_INSCRICAO
        WHERE  CO_USUARIO                 = REC.CO_USUARIO;


        EXCEPTION
        WHEN OTHERS THEN
            V_RETORNO := '{"CPF":"'||REC.NU_CPF||'", "INSCRICAO": "'||REC.CO_INSCRICAO||'", "LIMINAR": "'||V_CO_LIMINAR||'", "TP_VAGA": "'||V_TP_VAGA_LIMINAR||'", "ERRO":"FALHA AO OCUPAR VAGA] - ['||SQLERRM||'] "}';
    END;
    END IF;
    ...
    BEGIN
        ...
        VALUES ( FIES_FINANCIAMENTO.SQ_H_FIN_CANDIDATO_PUBLICO.NEXTVAL, REC.CO_USUARIO, P_CO_SITUACAO_FINANCIAMENTO, NULL, SYSDATE, V_CO_PERFIL_USUARIO );
        --DBMS_OUTPUT.PUT_LINE('TH_FIN_CANDIDATO_PUBLICO - Total de Linhas Incluidas:'||SQL%ROWCOUNT||'.');

        EXCEPTION
        WHEN OTHERS THEN
            V_RETORNO := '{"CPF":"'||REC.NU_CPF||'", "INSCRICAO": "'||REC.CO_INSCRICAO||'", "ERRO":"FALHA ATUALIZAR A SITUACAO DA INSCRICAO] - ['||SQLERRM||'] "}';
        END;
    
    END IF;
    IF V_RETORNO IS NULL THEN
        V_RETORNO := '{"CPF":"'||REC.NU_CPF||'", "INSCRICAO":"'||REC.CO_INSCRICAO||'", "VAGA RECUPERADA COM SUCESSO":"CLUSTER('||V_CO_CLUSTER||') CURSO('||V_CO_CURSO||') TURNO('||V_CO_TURNO||') LINHA('||V_NU_LINHA||') TIPO('||V_TP_VAGA_LIMINAR||') LIMINAR ('||V_CO_LIMINAR||')"}';
    END IF;
      V_TOTAL        := V_TOTAL + 1;
